...
export default function Navbar() {
  ...
  return (
    <nav className="w-full fixed top-0 left-0 z-50 border-b border-white/10 bg-black/90 backdrop-blur-md transition-all">
          ...
          {user ? (
            <div className="flex items-center gap-4">
              {user.email === "senin-mail-adresin@gmail.com" && (
                <Link
                  href="/admin"
                  className="text-red-400 text-xs font-bold border border-red-400/30 px-2 py-1 rounded hover:bg-red-400/10 transition"
                >
                  ADMIN
                </Link>
              )}
            ...
            <div className="flex items-center gap-3">
              <Link
                href="/login"
                className="px-5 py-2 bg-green-600 hover:bg-green-500 rounded-full text-sm font-bold transition text-white shadow-lg"
              >
                Giriş Yap
              </Link>
              <Link
      ...
      <div
        className={`md:hidden bg-black border-b border-white/10 overflow-hidden transition-all duration-300 ${
          ...
          {user ? (
            <>
              {user.email === "senin-mail-adresin@gmail.com" && (
                <Link
                  ...
                  className="text-red-400 font-bold w-full text-center py-2 border border-red-900 rounded bg-red-900/10"
                >
                  ADMIN PANELİ
                </Link>
              )}
              ...
              <Link
                href="/profile"
                onClick={() => setIsMenuOpen(false)}
                className="text-white font-medium w-full text-center py-2 hover:bg-white/5 rounded"
              >
              ...
              <Link
                href="/favorites"
                onClick={() => setIsMenuOpen(false)}
                className="text-gray-300 w-full text-center py-2 hover:bg-white/5 rounded"
              >
                Kütüphanem
              </Link>
            ...
            <>
              <Link
                href="/login"
                onClick={() => setIsMenuOpen(false)}
                className="w-full bg-green-600 text-white font-bold py-3 rounded-lg text-center"
              >
                Giriş Yap
              </Link>
              <Link
                href="/register"
                onClick={() => setIsMenuOpen(false)}
                className="w-full text-gray-300 py-2 text-center"
              >
                Kayıt Ol
              </Link>
            </>
